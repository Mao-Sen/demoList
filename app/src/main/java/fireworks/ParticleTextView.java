...
import static fireworks.PContext.dist;
import static fireworks.PContext.lerpColor;
import static fireworks.PContext.min;
...
public class ParticleTextView extends View {
    ...
    //
    class Particle {
        fireworks.PVector pos = new fireworks.PVector(0, 0);
        fireworks.PVector vel = new fireworks.PVector(0, 0);
        fireworks.PVector acc = new fireworks.PVector(0, 0);
        fireworks.PVector target = new fireworks.PVector(0, 0);
        ...
        void move() {
            ...
            fireworks.PVector towardsTarget = new fireworks.PVector(this.target.x, this.target.y);
            towardsTarget.sub(this.pos);
            towardsTarget.normalize();
            towardsTarget.mult(this.maxSpeed*proximityMult);

            fireworks.PVector steer = new fireworks.PVector(towardsTarget.x, towardsTarget.y);
            steer.sub(this.vel);
            steer.normalize();
        ...
        void kill() {
            if (! this.isKilled) {
                ...
                fireworks.PVector randomPos = generateRandomPos(width/2, height/2, (width+height)/2);
    ...
    fireworks.PVector generateRandomPos(int x, int y, float mag) {
        fireworks.PVector sourcePos = new fireworks.PVector(x, y);
        fireworks.PVector randomPos = new fireworks.PVector(random(0, width), random(0, height));

        fireworks.PVector direction = fireworks.PVector.sub(randomPos, sourcePos);
        direction.normalize();
        direction.mult(mag);
